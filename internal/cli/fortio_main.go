...
package cli // import "fortio.org/fortio/internal/cli"
...
var (
	...
	abortOnFlag            = flag.Int("abort-on", 0,
		"HTTP status code that if encountered aborts the run. e.g., 503 or -1 for socket errors.")
	...
	kafkaConsumerMetricsConfig = make([]kafkarunner.ConsumerServiceConfig, 0)
)
...
// FortioMain is the main entry point for Fortio.
// Accepts a FortioHook for backward compatibility.
// Use FortioMainWithConfig for full configuration including custom logger.
//
...
func FortioMain(hook bincommon.FortioHook) int {
	return FortioMainWithConfig(&bincommon.FortioConfig{Hook: hook})
}

// FortioMainWithConfig is the main entry point for Fortio with full configuration.
// Allows setting custom logger via config.LoggerSetup.
//
// Example with slog:
//
//	import "fortio.org/fortio/pkg/logger"
//
//	func main() {
//	    slogger := slog.New(slog.NewJSONHandler(os.Stdout, nil))
//	    os.Exit(cli.FortioMainWithConfig(&bincommon.FortioConfig{
//	        LoggerSetup: func() {
//	            logger.SetupSlogLogger(slogger)
//	        },
//	    }))
//	}
//
//nolint:funlen
func FortioMainWithConfig(config *bincommon.FortioConfig) int {
	if config == nil {
		config = &bincommon.FortioConfig{}
	}
	// Setup custom logger before anything else
	if config.LoggerSetup != nil {
		config.LoggerSetup()
	}
	hook := config.Hook
	flag.Func("P",
	...
	flag.Func("kafka-consumer-metrics-url",
		"Consumer service metrics URL with name, format: \"name url\" (can be repeated), "+
			"e.g., -kafka-consumer-metrics-url \"service1 http://host1:8080/metrics\" "+
			"-kafka-consumer-metrics-url \"service2 http://host2:8080/metrics\"",
		func(value string) error {
			parts := strings.SplitN(value, " ", 2)
			if len(parts) != 2 {
				return fmt.Errorf("invalid format, expected \"name url\", got %q", value)
			}
			name := strings.TrimSpace(parts[0])
			url := strings.TrimSpace(parts[1])
			if name == "" || url == "" {
				return fmt.Errorf("both name and url are required, got name=%q url=%q", name, url)
			}
			kafkaConsumerMetricsConfig = append(kafkaConsumerMetricsConfig, kafkarunner.ConsumerServiceConfig{
				Name: name,
				URL:  url,
			})
			return nil
	...
	case "server":
		...
		if *grpcPortFlag != disabled {
			fgrpc.PingServer(*grpcPortFlag, *healthSvcFlag, safecast.MustConv[uint32](*maxStreamsFlag), tlsOptions)
		}
		if *redirectFlag != disabled {
			fhttp.RedirectToHTTPS(*redirectFlag)
		}
	...
	case *kafkaBootstrapFlag != "" && *kafkaTopicFlag != "":
		...
		o := kafkarunner.RunnerOptions{
			...
			KafkaOptions: kafkarunner.KafkaOptions{
				...
				ConsumerServices: kafkaConsumerMetricsConfig,
			},
		}
