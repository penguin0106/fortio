...
// LoggerInterface определяет интерфейс для кастомного логгера.
// Реализуйте этот интерфейс для интеграции своего логгера (например, slog).
...
// FortioConfig содержит конфигурацию для запуска Fortio.
type FortioConfig struct {
	// Hook для кастомизации HTTP опций и runner опций
	Hook FortioHook
	// Logger - кастомный логгер (опционально)
	// Если nil, используется стандартный fortio.org/log
	Logger LoggerInterface
	// LoggerSetup - функция для настройки логгера перед запуском
	// Вызывается до инициализации CLI
	LoggerSetup func()
}
