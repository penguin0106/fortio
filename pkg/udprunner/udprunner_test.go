...
func TestUDPRunnerBadDestination(t *testing.T) {
...
func TestUDPRunner(t *testing.T) {
	addr := fnet.UDPEchoServer("test-echo-runner", ":0", false)
	...
	UDPOk := res.RetCodes[UDPStatusOK]
	if totalReq != UDPOk {
...
func TestUDPNotLeaking(t *testing.T) {
	...
	addr := fnet.UDPEchoServer("test-echo-runner", ":0", true)
	numCalls := 100
	opts.NumThreads = numCalls / 2 // make 2 calls per thread
	opts.Exactly = int64(numCalls)
	opts.QPS = float64(numCalls) / 2 // take 1 second
	opts.Destination = fmt.Sprintf("localhost:%d", addr.(*net.UDPAddr).Port)
	// Warm up round 1
	_, err := RunUDPTest(opts)
	if err != nil {
		t.Error(err)
		return
	}
